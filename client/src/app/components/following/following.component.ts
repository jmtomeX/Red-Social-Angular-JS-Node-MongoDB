...
export class FollowingComponent implements OnInit, DoCheck {
  ...
  constructor(
    ...
    this.title = 'Infectados seguidos por ';
    ...
    this._followService.getFollowing(this.token, user_id, page).subscribe(
      response => {
        if (!response.follows) {
          ...
          console.log("Metodo getFollows " + response)
          this.total = response.total;
          this.following = response.follows;
          this.pages = response.pages;
          this.follows = response.users_following;
          console.log("Metodo getFollows " + this.follows);
          if (page > this.pages) {
            // carga la página 1 de usuarios
            this._router.navigate(['/siguiendo/', this.user_page_id, 1]);
          }
        }
      },
      error => {
